...
function redir(fam,raz) {
    
    //alert(fam);
	localStorage.setItem('ruc', fam);
	localStorage.setItem('razon', raz);
	window.close()	
}
function dd2(ff)
...
<iframe  width="100%" style="display:block;" src="hlpbuscaPROVREAL_mm.asp" id="body10" name="body10" scrolling="no" frameborder="0" height="100px"></iframe>
...
<%	'*****************************
  ...
  <%' MUESTRA EL GRid (2 colorES) %>
    ...
    <tr  bgcolor="<% if CONT mod 2  = 0 THEN 
	            ...
	            onclick="dd('<%=(cont)%>')" id="fila<%=Trim(Cstr(cont))%>" ondblclick="redir('<%=trim(RS.FIELDS.ITEM(0))%>','<%=trim(RS.FIELDS.ITEM(1))%>')" >       
