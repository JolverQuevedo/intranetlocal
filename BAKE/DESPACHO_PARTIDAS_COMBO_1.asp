...
<table id="TABLA" width="100%" align="center" bgcolor="<%=application("color2")%>"  cellpadding="0"  cellspacing="1"  border="1" >
   <tr bgcolor="<%=APPLICATION("TITULO")%>"  align="center" >        
    ...
    <%DO WHILE NOT RS.EOF%>
        ...
        IF RS2.RECORDCOUNT > 0 THEN RS2.MOVEFIRST%>
         <tr  id="fila<%=cont%>"  >
                <td align="center" class="AMERICANnavy33" ><%=trim(rs2("PO"))%></td> 
                <td align="center" class="AMERICANnavy33" ><%=trim(rs2("combo"))%></td>
             
                <td align="right"  class="AMERICANnavy33" style="padding-right:5px;"><%=FORMATNUMBER(rs2("qty"),0,,,TRUE)%></td>
                <%for i= 0 to 9 %>
                <%dato = "t"&i %>
                <%POTO = "C"&I %>
                    <%if CDBL(rs2(POTO)) > 0 then %>
                        <td align="center"  class="AMERICANwhite10" style="color:navy" ><%=rs2(dato)%></td>
                    <%END IF%>
                <%next%>
            ...
            <tr  id="fila<%=Trim(Cstr(cont))%>"  valign="top">
               ...
               <%CA = CDBL(rs2("C0")) + CDBL(rs2("C1"))+CDBL(rs2("C2"))+CDBL(rs2("C3"))+CDBL(rs2("C4"))+CDBL(rs2("C5")) +CDBL(rs2("C6"))+CDBL(rs2("C7"))+CDBL(rs2("C8"))+CDBL(rs2("C9"))%>
                    ...
                    <td align="right"  class="AMERICANnavy333" style="padding-right:5px;"><input id='po<%=cont%>' name='po<%=cont%>' value='<%=FORMATNUMBER(CA,0,,,TRUE)%>'   class="BARRA333" style="width:100%; text-align:left" maxlength="100" disabled/></td>
                     <%for i= 0 to 9 %>
                        <%dato = "C"&i %>
                        <%if CDBL(rs2(dato)) > 0 then %>
                            <td align="center"  class="AMERICANnavy333" ><input id='po<%=cont%>qy<%=i%>' name='po<%=cont%>qy<%=i%>' value='<%=rs2(dato)%>'  class="BARRA333" style="width:100%; text-align:left" maxlength="100" /></td>
                        <%END IF%>
                    <%next%>
