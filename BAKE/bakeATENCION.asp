...
' hay que ver cual es el ALMACEN del SERVICIO para compararlo con el de la OC
...
' DATOS DE CANTIDADES ....OC_NCANORD-OC_NCANSAL
CAD =   " Select OC_CITEM, OC_CCODIGO, OC_CCODREF, OC_CDESREF, OC_CUNIDAD, CONVERT(NUMERIC(13,3),OC_NCANORD) AS OC_NCANORD,OC_NPREUN2,  " & _
        " CONVERT(NUMERIC(13,3), (OC_NCANORD*"&TOLERANCIA&")-ISNULL((select  sum(case when c5_ctipmov = 'E' then c6_ncantid else 0 end) from RSFACCAR..al0001movc AS A inner join   " & _
        ...
        " (select sum(c6_ncantid) from RSFACCAR..al0001movc AS A inner join                                                             " & _
        " RSFACCAR..al0001movd AS B on c5_cnumdoc = c6_cnumdoc and c5_ctd = c6_ctd and c5_calma = c6_calma where C5_CNUMord='"&pos&"'   " & _
...
RS.OPEN CAD, CNN
    ...
    %>
		<script type="text/jscript" language="jscript">
		    ...
		    pun = FormatNumber(parseFloat('<%=rs("OC_NPREUN2")%>'), 2, true, true, true, true)
...
CAD =   "Select AR_CFSERIE, AR_CUNIDAD,  AR_CFSERIE, AR_CFLOTE,AR_NPRECI2 , AR_NPRECI1  AR_CMONVTA,                                                                              " & _
		...
		if RS.RECORDCOUNT > 0 then
            ...
            response.write("++++++++++++++++")
            response.write("<br>")
            response.write( left(trim(ref),1) )
            response.write("<br>")
            %>
            <script type="text/jscript" language="jscript">
			    mon = '<%=rs("cod_MON")%>'
